...
{
    /* Based on ANSI-C implementation: https://github.com/rafaelmmoreira/PanTompkinsQRS */
    internal class QrsDetector
    {
        private ISourceBlock<double> _inputBlock;
        private ITargetBlock<int> _outputBlock;
        ...
        * increase the buffers' size as you see fit to extract different kinds of infor-*
        ...
        public int WINDOWSIZE { get; set; }   // Integrator window size, in samples. The article recommends 150ms. So, FS*0.15.
                               // However, you should check empirically if the waveform looks ok.

        public int FS { get; set; } = 360;          // Sampling frequency.

        public int BUFFSIZE { get; set; } = 600;    // The size of the buffers (in samples). Must fit more than 1.66 times an RR interval, which
                               // typically could be around 1 second.

        const int DELAY = 22;        // Delay introduced by the filters. Filter only output samples after this one.
        ...
        */
        public void Init(ISourceBlock<double> sampleSource, ITargetBlock<int> outputBlock, int samplingRate)
        {
            _inputBlock = sampleSource;
            _outputBlock = outputBlock;

            FS = samplingRate;
            BUFFSIZE = samplingRate * 3; // 3sec
            WINDOWSIZE = (int)(FS * 0.15); // 150ms
        }
        ...
        */
        private void Output(int outputValue)
        {
            //_outputBlock.Post(outputValue);
        }

        private void OutputRR(int rr)
        {
            _outputBlock.Post(rr);
        }
        ...
        */
        public async Task PanTompkins(CancellationToken cancellationToken = default)
        {
            ...
            double[] signal = new double[BUFFSIZE],
                dcblock = new double[BUFFSIZE],
                lowpass = new double[BUFFSIZE],
                highpass = new double[BUFFSIZE],
                derivative = new double[BUFFSIZE],
                squared = new double[BUFFSIZE],
                integral = new double[BUFFSIZE],
                outputSignal = new double[BUFFSIZE];
            ...
            double rravg1, rravg2 = 0, rrlow = 66, rrhigh = 400, rrmiss = 0;
            ...
            ulong i, j, c, sample = 0, lastQRS = 0; double lastSlope = 0, currentSlope = 0;
            ...
            double peak_i = 0, peak_f = 0, threshold_i1 = 0, threshold_i2 = 0,
            ...
            do
            {
                ...
                if (sample >= (ulong)BUFFSIZE)
                {
                    for (i = 0; i < (ulong)BUFFSIZE - 1; i++)
                    {
                        signal[i] = signal[i + 1];
                ...
                if (!await _inputBlock.OutputAvailableAsync())
                {
                    break;
                }

                signal[current] = _inputBlock.Receive();
                ...
                if (current >= 32)
                    highpass[current] += lowpass[current - 32] / 32;
                ...
                if ((integral[current] >= threshold_i1) && (highpass[current] >= threshold_f1))
                {
                    ...
                    else
                    {
                        ...
                        threshold_f2 = 0.5 * threshold_f1;
                        qrs = false;
                        outputSignal[current] = 0;
                        if (sample > (ulong)(DELAY + BUFFSIZE))
                            Output((int)integral[0]);
                ...
                else
                {
                    ...
                    if ((sample - lastQRS > (ulong)rrmiss) && (sample > lastQRS + (FS / 5.0)))
                    {
                        for (i = current - (sample - lastQRS) + (ulong)(FS / 5.0); i < current; i++)
                        {
                            if ((integral[i] > threshold_i2) && (highpass[i] > threshold_f2))
                            {
                                ...
                                if ((currentSlope < (lastSlope / 2.0)) && (i + sample) < lastQRS + 0.36 * lastQRS)
                                {
                                    qrs = false;
                                }
                                else
                                {
                                    ...
                                    rr1[7] = (int)(sample - (current - i) - lastQRS);
                                    qrs = true;
                ...
                if (qrs && rr2[7] >= rrlow && rr2[7] <= rrhigh)
                {
                    OutputRR(rr2[7]);
                }

            } while (!cancellationToken.IsCancellationRequested && !_inputBlock.Completion.IsCompleted);

            // Output the last remaining samples on the buffer
            for (i = 1; i < (ulong)BUFFSIZE; i++)
                Output((int)integral[i]);
        }
    }
}
